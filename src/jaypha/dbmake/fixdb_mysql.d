...
module jaypha.dbmake.fixdb_mysql;
...
void main(string[] args)
{
  ...
  auto queries = get_queries(database_def, db_connection);
  ...
  foreach (query; queries)
  {
    if (query)
    {
      ...
      if (!dry_run) { db_connection.query(query); }
    }
  }
...
string[] get_queries(ref DatabaseDef def, ref DB db)
{
  // For each table get queries for the table
  string[] new_tables;
  string[] old_names;
...
string get_create_table_sql(ref TableDef def, ref DB db)
{
  ...
  {
    primary = "id";
    items.put("`id` int(11) unsigned not null auto_increment");
  }
...
string get_type_sql(ref ColumnDef def, ref DB db)
{
  string s;
...
string get_column_sql(ref ColumnDef column_def, ref DB db)
{
...
string get_index_sql(ref IndexDef index_def, ref DB db)
{
  string s;
...
(
  ref TableDef new_def, ref TableDef old_def, ref DB db
)
{
  auto s = appender!(string[])();
  auto d = appender!(string[])(); // For drops, which need to be done first.
  ...
  if (!new_def.no_id && !("id" in old_def.columns))
  {
    ...
    s.put("add column `id` int(11) unsigned not null auto_increment");
  }
  ...
  foreach (name, column_def; new_def.columns)
  {
    ...
    else if (name in old_def.columns)
    {
      ...
      if (change)
      {
        ...
        s.put("modify "~get_column_sql(column_def, db));
      }
    }
    else
    {
      ...
      s.put("add column "~get_column_sql(column_def, db));
    }
  }
  ...
  foreach (name, column_def; old_def.columns)
  {
    if (name == "id")
    {
      if (new_def.no_id)
      {
        ...
        d.put("drop column 'id'");
        d.put("drop primary key");
      }
    }
    else if (find(names,name).empty) 
    {
      if (!quiet) writeln("  removing "~name);
      d.put("drop column `"~name~"`");
    }
  }
  ...
  foreach (name,idx; old_def.indicies)
  {
    ...
    else
    {
      ...
      if (!(name in new_def.indicies))
      {
        ...
        d.put(format("drop index `%s`", name));
      }
    }
  }
  ...
  foreach (name,idx; new_def.indicies)
  {
    if (!(name in old_def.indicies))
    {
      ...
      s.put("add " ~ get_index_sql(idx, db));
    }
    else
    {
      if
      ...
      )
      {
        ...
        d.put("drop index `"~name~"`");
        s.put("add "~get_index_sql(idx,db));
      }
    }
  }
  auto items = d.data ~ s.data;
...
TableDef extract_table(string name, ref DB db)
{
  TableDef table;
  table.name = name;
